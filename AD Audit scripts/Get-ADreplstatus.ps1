#Variable
$report_path = "D:\Scripts\AD\Audit\Report"
...
Function send_mail([string]$message, [string]$subject) {
    $emailFrom = "sender@BRP.com"
    $emailTo = "Francois.Fournier4@BRP.com"
    $emailCC = "cc@BRP.com"
...
if ($array | Where-Object { $_.repl_status -notlike "*successful*" }) {
    $message += $array | Where-Object { $_.repl_status -notlike "*successful*" } | Select-Object source_dc, nc, destination_dc, repl_status | ConvertTo-Html -Head $html_head -Property source_dc, nc, destination_dc, repl_status
}
else {
...
$message += $array | Where-Object { $_.repl_status -like "*successful*" } | Select-Object source_dc, nc, destination_dc, repl_status | ConvertTo-Html -Head $html_head -Property source_dc, nc, destination_dc, repl_status
