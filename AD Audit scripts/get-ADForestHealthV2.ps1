 <#
.SYNOPSIS
	script leverages Repadmin.exe and DCdiag.exe across the entire forest or domain to help analysis and troubleshooting.

.DESCRIPTION
	Analysis of Forest

.PARAMETER Domain
	Specifies the domain to test

.PARAMETER FilePath
	Path for file of domains
    
.PARAMETER report
	Switch to write report to file. The default path is the current directory (.).

.INPUTS
	None

.OUTPUTS
	TXT:  $DataLocation\ADREPORT for <Domain> MM.dd.yyyy.HH.mm.ss.txt


.Example
    Get-ADForestHealth.ps1 -Domain <Name> -report
.Example
    Get-ADForestHealth.ps1 -FilePath <Name>

.Notes
    NAME:   Get-ADForestHealth
...
Function Get-ForestDNSAnalysis_Local{
                ...
                $adreporttxt = "ADREPORT for " + $domain +" "+ (Get-Date -Format MM.dd.yyyy.hh.mm.ss) +".txt"
...
Function Get-ForestDNSAnalysis{
				...
				$SRVLog2=$dcDiag -like "*Error details: 9003*"
				$CFLOG= $dcDiag -like "*Missing A record at DNS server*"
				$REP = invoke-command -computername $DC -scriptblock {repadmin /replsummary | Where-Object {$_ -ne ""}}
                ...
                $CPULOAD = invoke-command -computername $DC -scriptblock {Get-WmiObject win32_processor | Measure-Object -property LoadPercentage -Average | Select-Object Average }
