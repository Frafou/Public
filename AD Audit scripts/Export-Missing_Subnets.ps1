<#
    ...
    .PARAMETER EmailServerPort
        Specifies the port number to use to connect to the email server
    .PARAMETER KeepLogs
        Switch to keep the DC logs
    .PARAMETER HTMLReportPath
        Path of the Report files


    .EXAMPLE
        ./AD-SITE-Find_Missing_Subnets.ps1 -Verbose -EmailServer SMTP-CAVL.brp.local -EmailTo "Contact1@BRP.COM","Contact2@BRP.COM" -EmailFrom ADREPORT@BRP.COM -EmailSubject "Report - AD - Missing Subnets"

        This example will query all the Domain Controllers in the Active Directory and get the last 200 lines (Default) of each NETLOGON.log files. It will then send an email report to Contact1@BRP.COM and Contact2@BRP.COM.

        .EXAMPLE
        ./AD-SITE-Find_Missing_Subnets.ps1 -Verbose -EmailServer SMTP-CAVL.brp.local -EmailTo "Contact1@BRP.COM" -EmailFrom ADREPORT@BRP.COM -EmailSubject "Report - AD - Missing Subnets" -KeepLogs

        This example will keep all the log files.


    .NOTES
        NAME:   AD-SITE-Find_Missing_Subnets.ps1
        ...
        -Permission to write locally in the Output folder ($ScriptPath\Report)
...
PARAM (
    ...
    [Parameter(Mandatory = $False, HelpMessage = 'Specify the Destination Email Address')]
    [ValidatePattern("[a-z0-9!#\$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#\$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")]
    [String[]]$EmailTo,
...
BEGIN {
    TRY {
        ...
        $ReportPathOutput = $ScriptPath + '\Report'
        ...
        IF (-not (Test-Path -Path $ReportPathOutput)) {
            Write-Verbose -Message "[BEGIN] Creating the Output Folder : $ReportPathOutput"
            New-Item -Path $ReportPathOutput -ItemType Directory -ErrorAction 'Stop' | Out-Null
        }
...
PROCESS {
    TRY {
        FOREACH ($Domain in $Forest.Domains) {
            ...
            $DomainControllers = $domain | ForEach-Object -Process { $_.DomainControllers } | Select-Object -Property Name | Sort-Object name

            $i = 0
            $total = ($DomainControllers.count)
            Write-Verbose -Message "Processing $DCCount Domain Controllers"
            ...
            FOREACH ($dc in $DomainControllers) {
                ...
                Write-Progress -Activity 'Exporting DC logs' -Status "($i of $total)" -CurrentOperation "Exporting $DCName" -PercentComplete ($i / $total * 100)
                ...
                TRY {
                    ...
                    IF ((Test-Path -Path $path) -and ($null -ne (Get-Item -Path $path).Length)) {
                        ...
                        ELSE {
                            Write-Verbose -Message "[PROCESS] FOREST: $ForestName DOMAIN: $domainName - $DCName - NETLOGON File Empty !!"
                        }
                    }
                    ELSE {
                        Write-Warning -Message "[PROCESS] FOREST: $ForestName DOMAIN: $domainName - $DCName - NETLOGON.log is not reachable"
                    }
                    ...
                    IF ($FilesToCombine) {
                        ...
                        $importString | Where-Object -FilterScript { $_.Error -like '*NO_CLIENT_SITE*' } | Export-Csv -LiteralPath $scriptpathOutput\$ForestName-$dateformat-NOCLIENTSITE.csv -Append -NoTypeInformation
                        # Append Other Error File
                        $importString | Where-Object -FilterScript { $_.Error -notlike '*NO_CLIENT_SITE*' } | Export-Csv -LiteralPath $scriptpathOutput\$ForestName-$dateformat-OTHERERRORS.csv -Append -NoTypeInformation

                    }#IF File to Combine
                    ELSE {
                        Write-Verbose -Message '[PROCESS] Nothing to process'
                    }

                }#TRY
                CATCH {
                    Write-Warning -Message "$ForestName - $domainName - $DCName - Something wrong happened"
                    if ($ErrorOutFileNetLogon) {
                        Write-Warning -Message "$ForestName - $domainName - $DCName - Error with Out-File"
                    }
                }#CATCH
            }#FOREACH
        ...
        IF ($MissingSubnets) {
            $EmailBody += '<i>List of Active Directory client that cannot find their site.<br> You need to add these subnets in the Active Directory Sites And Services console</i>'
        ...
        ELSE {
            $EmailBody += '<i>No Missing Subnet(s) detected</i>'
        }
        ...
        ELSE {
            $EmailBody += '<i>No Other Error detected</i>'
        }
    ...
    FINALLY {
        ...
        # SAVE REPORT #
        ###############
        $EmailBody | Out-File -LiteralPath (Join-Path -Path $ReportPathOutput -ChildPath "Missing Subnet report for $ForestName-$dateformat-Report.html")
        ...
        if ($email) {
            ...
            #Send-MailMessage -From $EmailFrom -To $Emailto -Subject $EmailSubject -SmtpServer $EmailServer -port $EmailServerPort

            Send-MailMessage -From $EmailFrom -To $Emailto -Subject $EmailSubject -SmtpServer $EmailServer -Port $EmailServerPort -Body $FinalEmailBody #-Priority High -DeliveryNotificationOption OnSuccess, OnFailure -Attachments .\data.csv
